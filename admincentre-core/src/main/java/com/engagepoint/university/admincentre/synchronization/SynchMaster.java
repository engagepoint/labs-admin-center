...
public class SynchMaster {
    ...
    private class Receiver extends ReceiverAdapter {
        ...
        /**
         * Allows an application to write a state through a provided
         * OutputStream. After the state has been written the OutputStream
         * doesn't need to be closed as stream closing is automatically done
         * when a calling thread returns from this callback.
         *
         * @param output the OutputStream
         * @throws Exception if the streaming fails, any exceptions should be
         * thrown so that the state requester can re-throw them and let the
         ...
         * @throws Exception if the streaming fails, any exceptions should be
         * thrown so that the state requester can catch them and thus know what
         * happened
     ...
     * </ol>
     *
     * @param msg The message to be sent. Destination and buffer should be set.
     * A null destination means to send to all group members.
     * @throws Exception if
     * @exception IllegalStateException thrown if the channel is disconnected or
     * closed
     */
    ...
    /**
     *
     * @return <b>true</b> if operation done well, <b>false</b> if only one
     * member of cluster
    ...
    /**
     * Creates a sequence of operations which than could be used in push, pull,
    ...
    private List<CRUDPayload> sequance(MergeStatus mergeStatus, CRUDOperation crudOperation) {
        ...
        for (Pair<MergeStatus, AbstractEntity> pair : mergeList) {
            if (pair.getLeft() == mergeStatus) {
                ...
                sequance.add(new CRUDPayload(crudOperation, pair.getRight()));
                ...
                sequance.add(new CRUDPayload(CRUDOperation.UPDATE, parentNode));
            }
        }
    ...
    /**
     *
     * @return sorted list with pair of MergeStatus and entity both from cluster
     * and member
     */
    public List<Pair<MergeStatus, AbstractEntity>> merge() {
        ...
        common.values().retainAll(receivedState.values());
        Map<String, AbstractEntity> member
                = new HashMap<String, AbstractEntity>(cacheData);
        member.values().removeAll(receivedState.values());
        Map<String, AbstractEntity> cluster
                = new HashMap<String, AbstractEntity>(receivedState);
        cluster.values().removeAll(cacheData.values());
     ...
     * cluster name) from file. Connects to the cluster if cluster name was read
     * from file.
     */
